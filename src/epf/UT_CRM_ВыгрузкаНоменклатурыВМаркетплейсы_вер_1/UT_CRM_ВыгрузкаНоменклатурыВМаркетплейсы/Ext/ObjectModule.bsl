// отладка через РН.ТоварыНаСкладах:
// Вместо РС.РаспределениеЗапасов можно использовать РН.ТоварыНаСкладах - для отладки
// Для этого надо закомментировать методы:
//	ПолучитьВыборкуСвободныхОстатковСУчетомИзменений()
//	ПолучитьВыборкуСвободныхОстатков()
// и убрать слово "_заглушка" у методов:
//	ПолучитьВыборкуСвободныхОстатков_заглушка()
//	ПолучитьВыборкуСвободныхОстатковСУчетомИзменений_заглушка()
// Эти методы работают через получение доступных остатков методом
//	ТЗДоступныхЗаглушка()
// который читает РН.ТоварыНаСкладах.
...
//	Тип: Тип_значения
//
Функция ПолучитьВыборкуСвободныхОстатков_заглушка() Экспорт
...
Функция ПолучитьВыборкуСвободныхОстатков() Экспорт
	...
	// Запрос взят из обработки 
	//		ПолнаяВыгрузкаКаталогТоваров
	// ('C:\eCom\service_system\service\data\export\un\ПолнаяВыгрузкаКаталогТоваров.epf')
	   
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РаспределениеЗапасов.Номенклатура КАК НоменклатураНовая,
		|	СУММА(РаспределениеЗапасов.ВНаличии) КАК ВНаличии,
		|	СУММА(РаспределениеЗапасов.Свободно) КАК КоличествоНовое,     // это доступное количество
		|	СУММА(РаспределениеЗапасов.Резерв) КАК Резерв,
		|	РаспределениеЗапасов.Характеристика КАК ХарактеристикаНовая,
		|	РаспределениеЗапасов.Склад КАК СкладНовый
		|ИЗ
...
//
Функция ПолучитьВыборкуСвободныхОстатковСУчетомИзменений_заглушка(ТекДата, Маркетплейс) Экспорт
...
Функция ПолучитьВыборкуСвободныхОстатковСУчетомИзменений(ТекДата, Маркетплейс) Экспорт
	...
	Запрос.Текст =
		...
		|	СУММА(РаспределениеЗапасов.ВНаличии) КАК ВНаличии,
		|	СУММА(РаспределениеЗапасов.Свободно) КАК Доступно,     // это доступное количество
		|	СУММА(РаспределениеЗапасов.Резерв) КАК Резерв,
...
Процедура ВыгрузитьВМаркетплейсы() Экспорт
	...
	Для каждого МаркетПлейс Из ТЗМаркетплейсов.ВыгрузитьКолонку("Ссылка") Цикл
		...
		Выборка2 = ВыбратьВыгружаемыеПозицииИзРегистра(ТекДата, МаркетПлейс);
		
		Пока Выборка2.Следующий() Цикл
			
			ВыгрузитьВМаркетплейс(Выборка2, МЗ, МаркетПлейс, ТекДата);
...
//	Параметр1 	- Тип1 - 
//
Процедура ВыгрузитьВМаркетплейс(Выборка, МЗ, Маркетплейс, ТекДата)
	...
	// выгрузить позицию
	//...  
	ЗаписьЖурналаРегистрации("ВыгрузкаВМП",УровеньЖурналаРегистрации.Информация, ,,"Выгружается позиция: "+строка(Выборка.Номенклатура));
	
	
	// отключить флаг выгрузки
	МЗ.Период 			= ТекДата;
